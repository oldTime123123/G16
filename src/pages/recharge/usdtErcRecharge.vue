			...
			<view class="mt55">
				...
				<view class="mainBox">
					<view class="flex">
						...
						<input type="text" :disabled="needAmount>0"  placeholder-class="plo" focus :placeholder="t('mine.m_s7')" v-model="inputNum">
					</view>
...
<script setup>
	...
	const goOrder1 = () => {
		...
		request({
			url: 'finance/usdtErc/recharge/submit',
			methods: 'post',
			data: data
		}).then(res => {
	...
	const getData = () => {
		request({
			url: 'finance/usdtErc/recharge/index',
			methods: 'get'
		}).then(res => {
			if (res.order) {
				uni.navigateTo({
