<template>
	<!-- oticebar :text="topNotice" :left-icon="store.$s -->
			...
			<view class="mt55">
				...
				<view class="notice " style="background-color: #fff;" dir="ltr">
					<nut-noticebar :text="topNotice" :left-icon="store.$state.imgObj.labaIcon"
						background="rgba(251, 248, 220, 0)" color="#000">
					</nut-noticebar>
				</view>
				...
				<view class="mt38">
					...
					<view class="mt34 otpEl">
						...
						<view class="otp" v-if="!pageData.user_link" @click="changePage('../setting/bindUSDCerc')"
...
const getData = () => {
	request({
		url: 'finance/usdcErc/withdraw/index',
		methods: 'get',
	}).then(res => {
		pageData.value = res
...
const submitHandle1 = () => {
	...
	request({
		url: 'finance/usdcErc/withdraw/submit',
		methods: 'post',
		data: data
	}).then(res => {
		getData();
