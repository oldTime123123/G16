<template>
	<!-- ass="topBox center flex-col color0" :style=" -->
			...
			<view class="mt55">
				...
				<view class="mt38">
					...
					<view class="mt34 otpEl">
						...
						<view class="otp" v-if="!pageData.user_link" @click="changePage('../setting/bindUSDCbep')"
...
const getData = () => {
	request({
		url: 'finance/usdcBep/withdraw/index',
		methods: 'get',
	}).then(res => {
		pageData.value = res
...
const submitHandle1 = () => {
	...
	request({
		url: 'finance/usdcBep/withdraw/submit',
		methods: 'post',
		data: data
	}).then(res => {
		getData();
